...
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '2'  # 0 = all logs, 3 = only fatal errors
...
from r2r.agent_test_submit import GMapNavAgentSubmit
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    if 'reordering' in args.val_env_names[0]:
        best_val = {'reordering_val_unseen': {"spl": 0., "sr": 0., "state":""}}
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    if args.submit and 'test' in args.val_env_names:
        agent_class = GMapNavAgentSubmit
    else:
        agent_class = GMapNavAgent
    ...
    # if args.resume_file is not None:
    #     print("-"*25)
    #     iter_loaded = agent.load(args.resume_file)
    #     print("Loaded the listener model at iter %d from %s" % (
    #         iter_loaded, args.resume_file))
    ...
    for env_name, env in val_envs.items():
        ...
        if default_gpu:
            ...
            if env_name not in ['val_unseen_part_1', 'NavNuances_DC', 'NavNuances_LR', 'NavNuances_NU', 'NavNuances_RR', 'NavNuances_VM', 'test']:
            ...
            if args.submit:
                ...
                #         preds[k] = v.tolist()  # or v.item() if it's a scalar
