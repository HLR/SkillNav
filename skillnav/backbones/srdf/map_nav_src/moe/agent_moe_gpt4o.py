...
# Set for multi-Threads
NUM_THREADS = 4
MAX_RETRIES = 3

# For safely write into list
lock = threading.Lock()
...
def process_single_item(i, ob, instruction, previous_viewpoint_list, candidate_viewpoint_list, analyzer, args, logger, step):
    identify_result = analyzer.identify_instructions_with_gpt4o(
    ...
    skill_routing = analyzer.analyze_with_gpt4o(
        instruction,
        sub_instruction,
        reasoning,
    ...
    _, _, weights = analyzer.extract_from_response(skill_routing)
    ...
    if args.routing_weights_type == 'float':
        ...
        normalized_weights = weights  # fallback

    logger.info("-" * 20)
    logger.info(f"Step t: {step}")
    logger.info(f"Scan: {ob['scan']}")
    logger.info(f"Instruction ID: {ob['instr_id']}")
    logger.info(f"Original instruction: {instruction}")
    logger.info(f"Previous viewpoint list: {previous_viewpoint_list}")
    logger.info(f"Candidate viewpoint list: {candidate_viewpoint_list}")
    logger.info(f"Sub-instruction: {sub_instruction}")
    logger.info(f"Reasoning: {reasoning}")
    logger.info(f"Skill routing: {skill_routing}")

    return sub_instruction, normalized_weights
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.args.routing_mode != 'fixed':
                ...
                # self.logger.info(f"original paths: {original_paths}")
                ...
                # candidate_viewpoint_lists = [extract_cand_img(ob['viewpoint'], ob['scan'], self.vp_lookup) for ob in obs]
                ...
                with ThreadPoolExecutor(max_workers=self.args.batch_size) as executor:
                    ...
                    for i, ob in enumerate(obs):
                        futures.append(executor.submit (
                            ...
                            ob,
                            instructions[i],
                            previous_viewpoint_lists[i],
                            candidate_viewpoint_lists[i],
                            self.analyzer,
                            self.args,
                            self.logger,
                            t,
                    ...
                    for future in as_completed(futures):
                        try:
                            ...
                            if result:
                                with lock:
                                    sub_instructions.append(result[0])
                                    loaded_weights.append(result[1])
                            # sub_instruction, normalized_weights = future.result()
                            # if normalized_weights and sub_instruction:
                            #     with lock:
                ...
                '''
                for i, ob in enumerate(obs):
                    identify_result = self.analyzer.identify_instructions_with_gpt4o(instruction = instructions[i], previous_viewpoint_list= previous_viewpoint_lists[i])
                    sub_instruction = identify_result.get("Sub-instruction to be executed", "")
                    reasoning = identify_result.get("Reasoning", "")
                    skill_routing = self.analyzer.analyze_with_gpt4o(instructions[i], sub_instruction, reasoning, candidate_viewpoint_list = candidate_viewpoint_lists[i])
            ...
            if self.args.routing_mode == 'moe':
                ...
                for i, ob in enumerate(obs):
                    ...
                    # else :
                    #     weights_i = [1 if w > 0 else 0 for w in loaded_weights[i]]
                    #     # Use temporal agent as shared agent
                    #     weights_i[-1] = 1
                    weights_i = loaded_weights[i]
                    ### if shared
                    weights_i[-1] = 1
