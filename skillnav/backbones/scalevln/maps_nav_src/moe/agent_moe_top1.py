...
class GMapNavAgents(Seq2SeqAgent):
    ...
    def __init__(self, args, env, rank):
        ...
        self.router_server_url = getattr(self.args, "router_server_url", None)
        self._router_client = None
        self.localizer_llm, self.skill_llm = None, None

        if self.args.routing_mode == 'top1' and not self.router_server_url:
            if not self.args.debug and not self.args.localizer_gpu_id:
                # Load vLLM model for routing
                self.localizer_llm, self.skill_llm = load_two_vllms(
                    batch_size=self.args.batch_size,
                    localizer_model_ckpt="Qwen/Qwen2.5-VL-7B-Instruct",
                    skill_model_ckpt="Qwen/Qwen3-8B",
                )

            if self.args.localizer_gpu_id and self.args.skill_gpu_id:
    ...
    def _route_with_server(self, batch_inputs_router):
        if not self.router_server_url:
            raise RuntimeError("Router server URL is not configured.")
        if not batch_inputs_router:
            return [], []

        if self._router_client is None:
            self._router_client = httpx.Client(timeout=timeout)

        safe_payload = []
        for item in batch_inputs_router:
            safe_payload.append({
                "scan": str(item.get("scan", "")),
                "instr_id": str(item.get("instr_id", "")),
                "full_instruction": str(item.get("full_instruction", "")),
                "previous_viewpoint_list": [str(p) for p in item.get("previous_viewpoint_list", [])],
                "previous_sub_instruction_list": [str(s) for s in item.get("previous_sub_instruction_list", [])],
            })

        start_time = time.perf_counter()
        try:
            response = self._router_client.post(self.router_server_url, json=safe_payload)
            response.raise_for_status()
        except httpx.HTTPError as exc:
            self.logger.error(f"Router server request failed: {exc}")
            raise

        payload = response.json()
        expert_indices = payload.get("expert_indices", [])
        sub_instructions = payload.get("sub_instructions", [])
        latency = payload.get("latency_seconds", payload.get("latency_ms"))
        log_entries = payload.get("log_entries", [])

        elapsed = time.perf_counter() - start_time
        if self.logger:
            msg = f"Router server processed {len(batch_inputs_router)} items in {elapsed:.3f}s"
            if latency is not None:
                msg += f" (server reported {latency:.3f})"
        ...
        return expert_indices, sub_instructions
            ...
            def recover_state(name, model, optimizer):
                if name in states:
                    ...
                    if model_keys != load_keys:
                        self.logger.warning(f"NOTICE: DIFFERENT KEYS IN {name} for {resume_file}")
                        ...
                        self.logger.warning(f'Extra keys in {name} state_dict for {resume_file}: {", ".join(extra_keys)}')
                    ...
                    self.logger.warning(f"WARNING: Weights for {name} not found in {resume_file}")

            recover_state("vln_bert", vln_bert, vln_bert_optimizer)
            recover_state("critic", critic, critic_optimizer)
        else:
            self.logger.warning(f"WARNING: No state data found for {resume_file}")
            ...
            if ended[i]:                                            # Just ignore this index
                ...
                if ob['viewpoint'] == ob['gt_path'][-1]:
                    ...
                    if min_idx == self.args.ignoreid:
                        self.logger.warning('scan %s: all vps are searched' % (scan))
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        for t in range(self.args.max_action_len):
            ...
            if self.args.routing_mode == 'top1':
                ...
                if self.router_server_url:
                    expert_indices, sub_instructions = self._route_with_server(batch_inputs_router)
                else:
                    expert_indices, sub_instructions, _ = get_expert_indices(
                        self.localizer_llm,
                        self.skill_llm,
                        batch_inputs_router,
                        self.logger,
            ...
            if self.feedback == 'teacher':
                ...
                self.logger.error(f"Invalid feedback option: {self.feedback}")
