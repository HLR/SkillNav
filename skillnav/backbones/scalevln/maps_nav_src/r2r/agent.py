...
class GMapNavAgent(Seq2SeqAgent):
    ...
    def __init__(self, args, env, rank):
        ...
        if self.args.log_dir:
            ...
            # Set up logger
            log_file_path = os.path.join(self.args.log_dir, "nav_inputs_outputs.log")
    ...
    def _build_model(self):
        ...
        if self.args.resume_file:
            self.load_resumes(self.args.resume_file)
            ...
            if self.args.resume_file in self.resumes:
                self._load_model_weights(self.args.resume_file, self.vln_bert, self.vln_bert_optimizer, self.critic, self.critic_optimizer)
    ...
    def _language_variable(self, obs):
        ...
        # mask = np.zeros((len(obs), max(seq_lengths)), dtype=np.bool) # numpy==2.0.0
        mask = np.zeros((len(obs), max(seq_lengths)), dtype=bool) # numpy==1.26.2
    ...
    def _nav_gmap_variable(self, obs, gmaps):
        ...
        for i, gmap in enumerate(gmaps):
            ...
            if self.args.enc_full_graph:
                gmap_vpids = [None] + visited_vpids + unvisited_vpids # None = stop action
                gmap_visited_masks = [0] + [1] * len(visited_vpids) + [0] * len(unvisited_vpids)
            else:
                gmap_vpids = [None] + unvisited_vpids # None = stop action
    ...
    def rollout(self, train_ml=None, train_rl=False, reset=True):
        ...
        if self.args.debug:
            ...
            self.logger.info(f"language embedding {txt_embeds}")
        ...
        stopped = [False] * batch_size      # “have we already recorded the stop action?”
        stop_nodes = [None] * batch_size      # the viewpoint ID where we stopped
        ...
        for t in range(self.args.max_action_len):
            ...
            pano_embeds, pano_masks = self.vln_bert('panorama', pano_inputs) # pano_embeds: (bs, n_panos, dim), per candidate panorama viewpoint embeddings (including current viewpoint and candidates) for each batch element.
            
            avg_pano_embeds = torch.sum(pano_embeds * pano_masks.unsqueeze(2), 1) / \
                              torch.sum(pano_masks, 1, keepdim=True) #  For each visited node in the graph, we need a single embedding per node
            
            if self.args.debug:
                self.logger.info("-" * 20)
                self.logger.info(f"obs: {obs}")
                self.logger.info(f"step: {t}")
                self.logger.info(f"pano_inputs: {pano_inputs}")
                self.logger.info(f"pano_embeds: {pano_embeds}")
                self.logger.info(f"avg_pano_embeds: {avg_pano_embeds}")
            ...
            #     self.logger.info("-"*20)
            #     self.logger.info(f"nav_logits: {nav_logits}")
            ...
            for i, gmap in enumerate(gmaps):
                if not ended[i]:
                    ...
                    gmap.node_stop_scores[i_vp] = { 
                        'stop': nav_probs[i, 0].data.item(), # stop_node is all visited node
                    }
            ...
            cpu_a_t = []  
            for i in range(batch_size):
                if a_t_stop[i] or ended[i] or nav_inputs['no_vp_left'][i] or (t == self.args.max_action_len): # stop condition
                # if a_t_stop[i] or ended[i] or nav_inputs['no_vp_left'][i] or (t == self.args.max_action_len - 1): # determines if an action should be set to None (which means no movement will happen)
                    cpu_a_t.append(None) # stop action
                    just_ended[i] = True
                else:
                    cpu_a_t.append(nav_vpids[i][a_t[i]])   # cpu_a_t is viewpoint , not point ids
            ...
            # Update the a_t: if cpu_a_t is None, write stop_node once; else write the normal action
            for i in range(batch_size):
                if cpu_a_t[i] is None:
                    # a stop‐action: only write it once
                    # (we know stopped[i] is now True, but we still want to write it
                    #  the very first time cpu_a_t[i] is None)
                    if not stopped[i]:
                        traj[i]['a_t'][t] = stop_nodes[i]
                        stopped[i] = True
                else:
                    # normal nav action
                    traj[i]['a_t'][t] = cpu_a_t[i]
