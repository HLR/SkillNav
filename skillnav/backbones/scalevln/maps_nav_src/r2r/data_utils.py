...
#                 print("Reordering split detected:", split)
...
def load_instr_datasets_tok(anno_dir, dataset, splits, tokenizer, is_test=True):
    ...
    for split in splits:
        ...
        with open(filepath) as f:
            ...
            if 'reordering' not in split:  # the official splits
                for item in new_data:
                    ...
                    if 'instructions_l' in item:
                        ...
                        token_ids = tokenizer(item["instruction"])['input_ids']
                        instr_encodings.append(token_ids)
                ...
                for item in new_data:
                    if 'original_instruction' in item:
                        # if 'instr_encodings' not in item:
                        new_item = dict(item)
                        new_item['instruction'] = item['original_instruction']
                        new_item['reorder_instruction'] = item['instruction']
                        del new_item['original_instruction']
                        
                        instr_encodings = []
                        if 'instructions' in new_item:
                            for instruction in new_item["instructions"]:
                                # if 'instr_encodings' not in item:
                                token_ids = tokenizer(instruction)['input_ids']
                                instr_encodings.append(token_ids)
                        else:
                            token_ids = tokenizer(item["instruction"])['input_ids']
                            instr_encodings.append(token_ids)
                        new_item.update({"instr_encodings": instr_encodings})
