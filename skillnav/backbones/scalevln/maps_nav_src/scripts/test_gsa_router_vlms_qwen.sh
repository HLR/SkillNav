...
ROUTER_MODEL_NAME="${ROUTER_MODEL_NAME:-Qwen/Qwen2.5-VL-7B-Instruct}"
...
if ! command -v python3 >/dev/null 2>&1; then
  echo "python3 is required to probe router server health." >&2
  exit 1
fi
...
CUDA_VISIBLE_DEVICES="${AGENT_GPU}" python "${PROJECT_ROOT}/moe/main_nav_moe_top1.py" $flag \
  ...
  --val_env_names test_non_residential_scene_mp3d_filtered_part_1_8cases \
  --batch_size "${bs}"
